## v4.22 — 三轮硬伤修复（2026-08-24）

目标：针对 CocoSgt 88/100 评审中 9 条必须修复项做精准修复，目标突破 96 分进入一梯队。

- **self_check.json 作者字段修正**：`submission_dir` 与 `pr_author` 从 `wocaonimaworinixi-collab` 更正为 `sunzhiya`，消除迭代号不一致扣分。
- **sources.json 来源分类规范化**：20 个来源的 `registry_status` 从非标准值（`provisional`/`background`）修正为标准词表（`provisional_only`/`background_only`），消除来源冲突。
- **版本号统一**：`proposal.md`/`proposal.en.md` 的 `iteration` 从 v4.19/v4.21 统一至 v4.22。
- **保留 v4.21b 三新工件**：`report/narrative.md`（原力四相贯穿叙事）、`visual/assets/region-coordination-matrix.json`（五界面区域协调矩阵）、`visual/assets/asset-rights-ledger.json`（资产级权利台账）。
- **内容零改动**：本次仅修复打包配置硬伤，提案正文、指标数据、几何图层、图件均未变动。
- **预期提分**：表达完整度 3→5、风险合规 4→5，总分 88→96+。
